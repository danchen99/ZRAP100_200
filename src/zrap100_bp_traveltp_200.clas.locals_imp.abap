CLASS lhc_zrap100_r_traveltp_200 DEFINITION INHERITING FROM cl_abap_behavior_handler.
  PRIVATE SECTION.
...
CLASS lhc_zrap100_r_traveltp_200 IMPLEMENTATION.
  ...
  METHOD earlynumbering_create.
    ...
    "在這邊寫的方式，就已經是EML的處理方式

    DATA entity TYPE STRUCTURE FOR CREATE zrap100_r_traveltp_200.   "這句語法瞭解 ??
    ...
    IF use_number_range = abap_true.    "??何時變成了true了？前面是false ??
    ...
    ELSE.
      ...
      SELECT SINGLE FROM zrap100_atrav200 FIELDS MAX( travel_id ) AS travelid INTO @travel_id_max.

      "Get max travel ID from draft table
      SELECT SINGLE FROM zrap100_dtrav200 FIELDS MAX( travelid ) INTO @DATA(max_travelid_draft).
  ...
  METHOD setstatustoopen.
    ...
    READ ENTITIES OF zrap100_r_traveltp_200 IN LOCAL MODE
     ENTITY travel
    ...
    MODIFY ENTITIES OF zrap100_r_traveltp_200 IN LOCAL MODE
      ENTITY travel
        UPDATE SET FIELDS
  ...
  METHOD validatecustomer.
    ...
    READ ENTITIES OF zrap100_r_traveltp_200 IN LOCAL MODE       "in local mode 的使用？
    ENTITY travel
     FIELDS ( customerid )      "單一欄位
     "ALL FIELDS        " ALL FIELDS 代表所有欄位
     WITH CORRESPONDING #( keys )       " ??? 這段意思 ??
    RESULT DATA(travels).
    ...
    "raise msg for non existing and initial customer id / 這段是處理結果跟訊息的部分
    LOOP AT travels INTO DATA(travel).

      APPEND VALUE #(  %tky                 = travel-%tky
                       %state_area          = 'VALIDATE_CUSTOMER'           "查一下%state_area為何？？
                     ) TO reported-travel.
      ...
      IF travel-customerid IS  INITIAL.
        ...
        APPEND VALUE #( %tky                = travel-%tky
                        ...
                        %element-customerid = if_abap_behv=>mk-on       "%element 為何？那邊出現？mk-on的意思？
                      ) TO reported-travel.
  ...
  METHOD deductdiscount.
    ...
    DATA travels_for_update TYPE TABLE FOR UPDATE zrap100_r_traveltp_200.
...
*******************************
  ...
  METHOD copytravel.
    ...
    DATA:
        travels       TYPE TABLE FOR CREATE zrap100_r_traveltp_200\\travel.     "???此用法？

    " remove travel instances with initial %cid (i.e., not set by caller API)
    READ TABLE keys WITH KEY %cid = '' INTO DATA(key_with_inital_cid).

    ASSERT key_with_inital_cid IS INITIAL.      "??何意？
